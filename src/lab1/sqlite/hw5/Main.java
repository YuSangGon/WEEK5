	...
	public static void main(String[] args) {
		...
		try {
			...
			String sql2 = "insert into g_artisits (name, a_type, a_year, debut, regdate)"
				+ "values ('레드벨벳', '여성', '2010년대', '2013년', datetime('now','localtime'))";
			int cnt = stat2.executeUpdate(sql2);
			if(cnt > 0)
				System.out.println("새로운 데이터가 추가되었습니다!");
			else
				System.out.println("[Error] 데이터 푸가 오류!");
			stat2.close();
			
			System.out.println("\n*** 데이터 수정 ***");
			Statement stat3 = con.createStatement();
			String sql3 = "update g_artisits set a_year = '2000년대, 2010년대, 2020년대' "
					+ "where id = 1";
			...
			if(cnt4 > 0)
				System.out.println("데어터가 삭제되었습니다!");
			else
